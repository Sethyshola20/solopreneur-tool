...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
    return runWithAuthAPI(async (user) => {
        ...
        const updatedDevis = await db.transaction(async (tx) => {
            // Update Devis
            const [updated] = await tx.update(devis)
                .set({
            ...
            if (!updated) {
                throw new Error("Devis not found or unauthorized");
            }

            // Replace Items (Delete all and re-insert)
            // In a more complex app, you might want to diff and update.
            await tx.delete(devisItems).where(eq(devisItems.devisId, id));
        ...
        return successResponse(updatedDevis);
    });
}
