...
export function DevisForm({ visible, onClose, initialData, devisId }: DevisFormProps) {
    ...
    const { control, handleSubmit, reset, watch, formState: { errors } } = useForm<DevisSchema>({
    ...
    const onSubmit = async (data: DevisSchema) => {
        ...
        try {
            ...
            showToast.success(devisId ? 'Devis mis à jour !' : 'Devis créé !');
            onClose();
            reset();
        } catch (error: any) {
    ...
    return (
        <ModalComponent
            visible={visible}
            onClose={onClose}
            title={devisId ? 'Modifier le devis' : 'Nouveau devis'}
        >
